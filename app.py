...
# Replace compute_scan_for_ticker to route through desktop-like logic for parity
try:
    _old_compute = compute_scan_for_ticker
except NameError:
    _old_compute = None
def compute_scan_for_ticker(ticker: str, params: dict) -> dict:
    if _pfa is not None:
        dirn = params.get("direction", "BOTH")
        if dirn == "BOTH":
            ...
            picks = [r for r in (ra, rb) if r]
            if not picks:
                return {}
            return sorted(
                picks,
                key=lambda r: (r.get("avg_roi_pct", 0.0), r.get("hit_pct", 0.0), r.get("support", 0), r.get("stability", 0.0)),
                reverse=True
            )[0]
        else:
            return _desktop_like_single(ticker, params) or {}
    return (_old_compute(ticker, params) or {})

# -----------------------------
# Fallback placeholder (only if real engine is missing)
# -----------------------------
def _stub_scan_single(ticker: str, params: Dict[str, Any]) -> Dict[str, Any]:
    seed = sum(ord(c) for c in ticker) % 97
    hit = 50 + (seed % 40) * 0.6                 # ~50..74%
    roi = (math.sin(seed) * 8.0) + 1.5           # ~-6.5..+9.5
    support = 20 + (seed % 60)
    avg_dd = abs(math.cos(seed) * 3.0)
    stability = round((seed % 10) / 10, 3)

    if hit < float(params.get("scan_min_hit", 50)):
        return {}
    if roi <= 0:
        return {}

    direction = params.get("direction", "UP")
    rule = f"• EMA50_SLOPE_b > {params.get('slope_gate_pct', 0.02):.2f}  &  ATR_Z_b >= {params.get('atrz_gate', 0.10):.2f}"
    return {
        "ticker": ticker,
        "direction": "UP" if direction in ("UP", "BOTH") else "DOWN",
        "avg_roi_pct": round(roi, 2),
        "hit_pct": round(hit, 2),
        "support": support,
        "avg_tt": round((seed % 12) + 2.0, 1),
...
def compute_scan_for_ticker(ticker: str, params: Dict[str, Any]) -> Dict[str, Any]:
    if _real_scan_single:
        try:
            return _real_scan_single(ticker, params) or {}
        except Exception as e:
            print(f"[engine] error on {ticker}: {e!r}")
            return {}
    return _stub_scan_single(ticker, params)
