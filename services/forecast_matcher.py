...
def _trading_days(end: dt.date, lookback_years: int) -> List[dt.date]:
    start = end - dt.timedelta(days=lookback_years * 366)
    day = start
    days: List[dt.date] = []
    while day < end:
...
def _frame_slice(layout: Dict[str, Tuple[int, int]], frame: str, vector: np.ndarray) -> np.ndarray:
...
def find_similar_days(
    ...
    trading_days = _trading_days(target_day, lookback_years)
    ...
    for day in reversed(trading_days):
        ...
        for offset in _offset_sequence(tod_tolerance_min):
            ...
            if not passes:
                continue
            overall = _cosine(base_vec, cand_vec)
            ...
            match = MatchCandidate(candidate_ts, overall, frame_scores, outcomes, source)
    ...
    for match in matches:
        age_days = (asof_utc.date() - match.timestamp.date()).days
        weight = 1.0 - min(0.3, 0.3 * age_days / total_days)
        recency_weights.append(weight)
    recency_weight = float(np.mean(recency_weights)) if recency_weights else 0.7
